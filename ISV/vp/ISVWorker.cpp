...
{
    uint32_t bufCount = 1;
    if (mFrcRate != FRC_RATE_1X
            && index > 0)
            bufCount = (mFrcRate + 1) - (((mFrcRate == FRC_RATE_2_5X) ? (index & 1): 0));
    return bufCount;
}
...
status_t ISVWorker::configFilters(ISVFilterParameter *filterParam, uint32_t count)
{
    ...
    if (memcmp(&mFilterParam[0], filterParam, sizeof(ISVFilterParameter) * count) != 0) {
        ...
        mFiltersNum = count;
        //dump filters info
        for (uint32_t i = 0; i < mFiltersNum; i++) {
            ALOGV("%s: \t Filter [%02d]: FilterType %d, algorithm %d", __func__,
                    i,
                    mFilterParam[i].filter,
                    mFilterParam[i].algorithm);
            for (uint32_t j = 0; j < MAX_FILTER_PARAMETERS; j++)
                ALOGV("%s: value %.3f, ", __func__, mFilterParam[i].values[j]);
        }
        ret = setupFilters();
    }
...
status_t ISVWorker::setupFilters()
{
    ...
    VAProcFilterParameterBufferColorBalance ace;
    VAProcFilterParameterBufferTotalColorCorrection tcc[TCC_PARAM_SIZE];
    VAProcFilterParameterBufferFrameRateConversion frc;
    VABufferID deblockId, denoiseId, deintId, sharpenId, colorId, aceId, tccId, frcId, stdeId;
    uint32_t numCaps;
    ...
    VAProcFilterCapTotalColorCorrection tccCaps[TCC_PARAM_SIZE];
    VAStatus vaStatus;
    uint32_t i = 0, j = 0, featureCount = 0;

    if (mNumFilterBuffers != 0) {
        for (i = 0; i < mNumFilterBuffers; i++) {
            if (VA_STATUS_SUCCESS != vaDestroyBuffer(mVADisplay, mFilterBuffers[i]))
    ...
    for (i = 0; i < mFiltersNum; i++) {
        switch (mFilterParam[i].filter) {
            case ISVFilterDeblocking:
                numCaps = 1;
                ...
                deblock.value = mFilterParam[i].values[0];
            ...
            case ISVFilterNoiseReduction:
                ...
                denoise.value = mFilterParam[i].values[0];
                vaStatus = vaCreateBuffer(mVADisplay, mVAContext,
            ...
            case ISVFilterDeinterlacing:
                ...
                for (j = 0; j < numCaps; j++) {
                    bool getDIAlgorithm = false;
                    VAProcFilterCapDeinterlacing * const cap = &deinterlacingCaps[j];
                    // only support Bob and ADI deinterlacing currently
                    if (cap->type == VAProcDeinterlacingBob &&
                            mFilterParam[i].algorithm == DEINTERLACE_BOB) // desired Deinterlacing Type
                        getDIAlgorithm = true;
                    else if (cap->type == VAProcDeinterlacingMotionAdaptive &&
                            mFilterParam[i].algorithm == DEINTERLACE_ADI) // desired Deinterlacing Type
                        getDIAlgorithm = true;
                    if (!getDIAlgorithm)
                        continue;

                    deint.type = VAProcFilterDeinterlacing;
                    deint.algorithm = cap->type;
            ...
            case ISVFilterSharpening:
                ...
                sharpen.value = mFilterParam[i].values[0];
                vaStatus = vaCreateBuffer(mVADisplay, mVAContext,
            ...
            case ISVFilterColorBalance:
                ...
                // create parameter buffer
                for (j = 0; j < numCaps; j++) {
                    color[j].type = VAProcFilterColorBalance;
                    color[j].attrib = colorCaps[j].type;
                    color[j].value = mFilterParam[i].values[j];
                    featureCount++;
                }
                vaStatus = vaCreateBuffer(mVADisplay, mVAContext,
                ...
                mNumFilterBuffers++;
                break;
            case ISVFilterTotalColorCorrection:
                enum {kRed = 0, kGreen, kBlue, kCyan, kMagenta, kYellow, kCount};
                tcc[kRed].type = VAProcFilterTotalColorCorrection;
                tcc[kRed].attrib = VAProcTotalColorCorrectionRed;
                tcc[kRed].value = mFilterParam[i].values[kRed];

                tcc[kGreen].type = VAProcFilterTotalColorCorrection;
                tcc[kGreen].attrib = VAProcTotalColorCorrectionGreen;
                tcc[kGreen].value = mFilterParam[i].values[kGreen];

                tcc[kBlue].type = VAProcFilterTotalColorCorrection;
                tcc[kBlue].attrib = VAProcTotalColorCorrectionBlue;
                tcc[kBlue].value = mFilterParam[i].values[kBlue];

                tcc[kCyan].type = VAProcFilterTotalColorCorrection;
                tcc[kCyan].attrib = VAProcTotalColorCorrectionCyan;
                tcc[kCyan].value = mFilterParam[i].values[kCyan];

                tcc[kMagenta].type = VAProcFilterTotalColorCorrection;
                tcc[kMagenta].attrib = VAProcTotalColorCorrectionMagenta;
                tcc[kMagenta].value = mFilterParam[i].values[kMagenta];

                tcc[kYellow].type = VAProcFilterTotalColorCorrection;
                tcc[kYellow].attrib = VAProcTotalColorCorrectionYellow;
                tcc[kYellow].value = mFilterParam[i].values[kYellow];

                vaStatus = vaCreateBuffer(mVADisplay, mVAContext,
                        VAProcFilterParameterBufferType, sizeof(*tcc), TCC_PARAM_SIZE,
                        tcc, &tccId);
                CHECK_VASTATUS("vaCreateBuffer for color balance");
                mFilterBuffers[mNumFilterBuffers] = tccId;
                mNumFilterBuffers++;
                break;
            case ISVFilterFrameRateConversion:
                frc.type = VAProcFilterFrameRateConversion;
                mFrameRate = frc.input_fps = mFilterParam[i].values[0];
                mFrcRate = (ISV_FRC_RATE)(mFilterParam[i].algorithm);
                switch (mFrcRate){
                    case FRC_RATE_2X:
                    ...
                    case FRC_RATE_4X:
                        ...
                        break;
                    //FIXME: VSP driver can't support 1x frc
                    case FRC_RATE_1X:
                    default:
                        continue;
                }
                ...
                mFilterFrc = frcId;
                break;
            case ISVFilterSkinToneEnhancement:
                numCaps = 1;
                vaStatus = vaQueryVideoProcFilterCaps(mVADisplay, mVAContext,
                ...
                stde.value = mFilterParam[i].values[0];
                vaStatus = vaCreateBuffer(mVADisplay, mVAContext,
...
status_t ISVWorker::process(IISVBuffer* inputBuffer, Vector<IISVBuffer*> outputBuffer,
                             uint32_t outputCount, bool isEOS, bool syncMode, uint32_t flags)
{
    ...
    if (isEOS || syncMode) {
        VASurfaceID outputSurface = isEOS ? mPrevOutput : output[0];
        vaStatus = vaSyncSurface(mVADisplay, outputSurface);
...
status_t ISVWorker::fill(Vector<IISVBuffer*> outputBuffer, uint32_t outputCount)
{
...
uint32_t ISVWorker::getVppOutputFps() {
    ...
    if (FRC_RATE_2_5X == mFrcRate) {
        outputFps = mFrameRate * 5 / 2;
    } else {
        outputFps = mFrameRate * mFrcRate;
    }

    ALOGV("vpp is on in settings %d %d %d", outputFps,  mFrameRate, mFrcRate);
    return outputFps;
}
