 ...
 * <p>DisplayObserver.
 */
public class DisplayObserver extends IDisplayObserver.Stub {
    private static final String TAG = "ISVDPY";
    private static final boolean LOG = true;

    public  static final String MDS_OBSERVER = "mdsobserver";
    private static DisplayObserver mService;
    ...
    public static DisplayObserver main(Context cxt,
            WindowManagerPolicy.WindowManagerFuncs funcs) {
        if (mService != null) {
            return mService;
        }
        if (funcs == null) {
            Slog.w(TAG, "invalid WindowManagerFuns, MDS couldn't handle input envent");
        }
        mService = new DisplayObserver(cxt, funcs);
        try {
            Slog.v(TAG, "Create MDS Observer");
            ServiceManager.addService(MDS_OBSERVER, mService);
        } catch (Throwable e) {
            Slog.e(TAG, "Fail to create MDS Observer");
        }
        return mService;
    }

    public static IDisplayObserver getService() {
        IBinder b = ServiceManager.getService(MDS_OBSERVER);
        if (b == null)
            return null;
        return IDisplayObserver.Stub.asInterface(b);
    }

    private DisplayObserver(Context context,
            WindowManagerPolicy.WindowManagerFuncs funcs) {
        if (funcs == null) {
